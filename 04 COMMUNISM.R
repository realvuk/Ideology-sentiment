...
communism <- data.frame(tweets)

write_xlsx(communism, "D:/01 R Projects/03 COMMUNISM/communism.xlsx")

### DATA PREPARATIONS ###

libertarianism_tweets <- read_excel("D:/01 R Projects/03 COMMUNISM/communism.xlsx")
twt_txt <- communism_tweets$text
head(twt_txt)
...
communism_tweets <- read_excel("D:/01 R Projects/03 COMMUNISM/communism.xlsx")
twt_txt <- communism_tweets$text
communism_value <- get_nrc_sentiment(twt_txt)
communism_value[1:100, 1:10]
score <- colSums(communism_value[,])
score_df <- data.frame(score)
score_df
communism_score <- cbind(sentiment = row.names(score_df),
                              score_df, row.names= NULL)
print(communism_score)
ggplot(data = communism_score, aes(x = sentiment, y = score, fill = sentiment))+
